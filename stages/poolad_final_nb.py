...
#     of the English record, the same picture for the same song.
#
# This notebook masters the approved take to web loudness and PROVES the mastering cost nothing
# (the judge is deterministic, level-invariant, and reads Persian), gates the delivered bytes for
# register, words, continuity and fit, rebuilds the cover video under the new master, and writes
# the verification record the publication cites. It runs on the free CPU tier.
...
# LYRICS is the plain text (the judge's reference); SUNG is the fully vocalized text the singer
# was given — every consonant governed, so no short vowel is left to guess. They are the same
# words: stripping the marks from SUNG yields LYRICS, with the two colloquial contractions
# written as they are sung.
import re as _re
assert _re.sub(r"[\u064B-\u0652\u0670]", "", SUNG).strip() == \
       LYRICS.replace("لبه\u200cم", "لبم").replace("دسته\u200cم", "دستم").strip(), "sung text ≠ lyric"
for _fa, _en in zip(LYRICS.splitlines(), GLOSS.splitlines()):
    print(f"{_fa:<44}  {_en}" if not _fa.startswith("[") else _fa)
print("\n— as given to the singer, fully vocalized —\n")
print(SUNG)
...
# %% [markdown]
# `lib/songfit.py` at ArtaQuest/artamusic 2345be5 — does the lyric fit the clock. (Its
# measure/verdict are inlined as fit_measure/fit_verdict; the audio module above keeps the bare
# names.) This revision counts words in every script: the Latin-only pattern before it measured
# a thirty-line Persian lyric as zero words.
...
`fit_verdict()` returns the reasons to refuse, or [] — the same shape as `stillness.verdict`, so it
...
if __name__ == "__main__":
    ...
    m = fit_measure(lyric, dur, bpm, voiced_seconds=voiced)
    print(json.dumps(m, indent=1))
    for x in fit_verdict(m):
        print("REFUSED:", x)
...
# %%
# The lyric against the clock. With no audio the instrument ASSUMES a voiced fraction, and on
# Persian that assumption sits a hair over the anthem limit (the epic metre's eleven-syllable
# lines carry more particles than the English seven-syllable ones). The take exists here, so
# the gate is measured on ITS voiced seconds below, once the vocal stem is separated; this
# call only reports the plan.
fit_plan = fit_measure(LYRICS, 180.0, 112)
print("songfit (assumed voiced fraction):", json.dumps(fit_plan, ensure_ascii=False), flush=True)

def voiced_seconds_of(stem_path, hop_s=0.05, floor_db=-35.0):
    """Seconds of the separated vocal stem that carry voice: 50 ms frames whose RMS sits within
    floor_db of the stem's 95th-percentile loud frame. Pure measurement, no model."""
    x, sr = load(str(stem_path), mono=True)
    n = max(1, int(sr * hop_s)); m = len(x) // n
    fr = x[: m * n].reshape(m, n)
    rms_db = 20 * np.log10(np.sqrt((fr ** 2).mean(axis=1)) + 1e-9)
    ref = np.percentile(rms_db, 95)
    return float((rms_db > ref + floor_db).sum() * hop_s)
...
def fa_tokens(text):
    """Persian words as the judge compares them: stage tags dropped, ZWNJ split, Arabic-form
    letters folded to Persian, harakat and punctuation stripped."""
    t = re.sub(r"\[[^\]]*\]", " ", text)
...
def word_accuracy(mp3, stem=None):
    ...
    segs, _ = asr().transcribe(str(target), language="fa", beam_size=5, vad_filter=True,
                               vad_parameters=dict(min_silence_duration_ms=400),
                               chunk_length=20, condition_on_previous_text=False, temperature=0.0)
    hyp = fa_tokens(" ".join(s.text for s in segs))
    ref = fa_tokens(LYRICS)
...
take_seconds = float(subprocess.run(["ffprobe", "-v", "error", "-show_entries", "format=duration",
                                     "-of", "csv=p=0", str(TAKE)], text=True, capture_output=True).stdout.strip() or 180.0)
fit = fit_measure(LYRICS, take_seconds, 112, voiced_seconds=voiced_seconds_of(stem))
