# %% [markdown]
# # STEEL on ACE-Step, in the First Flame style — the male voice that HeartMuLa refused to give
#
...
# the male reference from the mounted kernel — the anchor that held across three records
_ref = sorted(Path("/kaggle/input").rglob("*KEEPTHEKEY*.mp3")) + sorted(Path("/kaggle/input").rglob("*.mp3"))
...
CAPTION = ("Dark cinematic epic that rises from embers. A long instrumental introduction: deep "
           "sub bass drone and sparse cinematic percussion, building slowly. A deep male voice "
           "enters low and close, half spoken, sitting inside the mix beneath the band. The "
           "choruses open upward, melodic and grave, and the drums grow heavier. One short "
           "passage leaves the voice almost alone over bass. The final choruses return twice as "
           "heavy with dense percussion, and the song ends at full force with no fade.")
...
for seed in (6001, 6002, 6003):
    ...
    assert found, f"{name}: no audio (rc {rc}) — {Path(f'/tmp/cli_{name}.txt').read_text()[-300:]}"
    mp3 = OUT / f"{name}.mp3"
    ...
    row = {"seed": seed, "male": ok,
           "register": {k: reg.get(k) for k in ("register","f0_hz","lead_hz","bands")},
           "style_distance": dist, "style": comp}
    report.append(row)
    (WORK / "probe.json").write_text(json.dumps(report, indent=1))
    print(f"  {name}: male={ok} lead={reg.get('lead_hz')} · style_distance={dist} · {comp}", flush=True)
    clock(f"{name} done")
