...
# STYLE-TRANSFER PROBE. The cover route measured a two-knob trap: strength pulls register
# toward the male source but drags the source's WORDS with it (words fell 71->70->63% as
# strength rose 0.6->0.8->0.9; register passed only at 0.8). text2music + reference_audio is
# the API's other conditioning mode — "set smaller (0.2) for style transfer" — where the lyric
# comes from TEXT (words stay free) and the reference only biases timbre. Three strengths.
CANDIDATES = [(SEED, 0.20), (4343, 0.35), (4444, 0.50)]
...
for seed, strength in CANDIDATES:
    ...
    conf.write_text(toml.dumps({
        ...
        "duration": DURATION, "inference_steps": 60, "guidance_scale": 7.5,
...
print("PROBE COMPLETE — verdicts in gate.json; no mastering in a probe", flush=True)
sys.exit(0)
...
wav, mp3 = OUT / "KEEPTHEKEY.wav", OUT / "KEEPTHEKEY.mp3"
...
for it in range(3):
    got = master_once(WINNER, ceiling)
...
ART_PROMPT = ("a house key on bare floorboards in an empty room, pale rectangles on the wall "
              "where pictures hung, late grey morning light through a curtainless window, "
              "cinematic, photorealistic, melancholic, ultra detailed")
