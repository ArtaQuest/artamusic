# STEEL — the animated cover: a seamless loop, SEAMLESS BY CONSTRUCTION.
#
# The earlier plan generated a clip and cross-dissolved its tail into its head. That hides a seam;
# it does not remove one, and it cannot fix the measured problem — LTX drifts ~25 luma units away
# from its conditioning image in three seconds, so a longer clip ends somewhere visibly unlike
# where it began and the dissolve just smears two different scenes together.
#
# LTXConditionPipeline solves it properly. LTXVideoCondition carries an image AND a frame_index,
# so the SAME still can be pinned at frame 0 and again at the final frame. The model then has to
# depart from the photograph and return to it, which makes the wrap exact and turns drift from a
# problem into a constraint the sampler is solving for. No dissolve, no ping-pong, no trimming.
#
# Frame budget is set by measurement, not preference: 73 frames generated comfortably here and
# 193 hit OutOfMemory, so 97 (~4 s at 24 fps) sits inside the proven range with headroom.
#
...
with open(ckpt, "rb") as f:
    for c in iter(lambda: f.read(1 << 22), b""):
        h.update(c)
print(f"checkpoint sha256 {h.hexdigest()[:24]} · {Path(ckpt).stat().st_size/2**30:.2f} GB", flush=True)
...
print("LTXConditionPipeline ready", flush=True)

W, H = 704, 480          # the better-looking of the two gate-1 resolutions
FPS = 24
NUM_FRAMES = 97          # 73 generated fine, 193 OOM'd — 97 is inside the proven range
...
NEG = ("worst quality, blurry, jittery, distorted, watermark, text, cartoon, cgi, "
       "camera movement, zoom, pan, cuts, scene change, morphing, melting, flicker")

img = Image.open(STILL).convert("RGB").resize((W, H), Image.LANCZOS)

# THE LOOP: the same photograph pinned at both ends. The tail condition is slightly weaker than
# the head so the return reads as the scene settling rather than snapping back to a freeze.
conditions = [
    LTXVideoCondition(image=img, frame_index=0, strength=1.0),
    LTXVideoCondition(image=img, frame_index=NUM_FRAMES - 1, strength=0.85),
]
...
print(f"generated {len(out)} frames in {gen_s}s", flush=True)

raw_mp4 = str(OUT / "steel_loop_raw.mp4")
export_to_video(out, raw_mp4, fps=FPS)

# ── deliver: VP9 (house codec) + h264 fallback, sized for a near-static scene ────────────
webm = OUT / "STEEL_cover_loop.webm"
mp4 = OUT / "STEEL_cover_loop.mp4"
sh(f"ffmpeg -v error -i '{raw_mp4}' -vf scale=1080:-2:flags=lanczos "
   f"-c:v libvpx-vp9 -crf 33 -b:v 0 -row-mt 1 -cpu-used 1 -g 240 -an '{webm}' -y")
sh(f"ffmpeg -v error -i '{raw_mp4}' -vf scale=1080:-2:flags=lanczos "
...
wrap = float(np.abs(luma[-1] - luma[0]).mean() * 255)      # last frame vs first: the loop point
typical = float(np.percentile(np.abs(np.diff(luma, axis=0)).reshape(len(a)-1, -1).mean(1)*255, 95))
verdict = {"frames": int(len(a)), "seconds": round(len(a)/FPS, 2),
           "gen_seconds": gen_s,
           "webm_mb": round(webm.stat().st_size/1048576, 2),
           "mp4_mb": round(mp4.stat().st_size/1048576, 2),
           "cuts": int((d > 0.10).sum()), "ti_mean": round(float(ti.mean()), 2),
           "wrap_delta": round(wrap, 2), "typical_frame_delta": round(typical, 2),
           "wrap_ratio": round(wrap / max(typical, 1e-6), 2),
           "res": f"{W}x{H}", "sha256_ckpt": h.hexdigest()[:24]}
(WORK / "loop_verify.json").write_text(json.dumps(verdict, indent=2))
print("\nVERIFY:", json.dumps(verdict), flush=True)
assert verdict["cuts"] == 0, f"the loop must not cut — {verdict['cuts']} found"
assert verdict["wrap_ratio"] < 2.0, (
    f"the wrap is visible: last-to-first delta {wrap:.2f} vs typical {typical:.2f}")
print("\nDONE — seamless by construction", flush=True)
